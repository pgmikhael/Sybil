...
def visualize_attentions(
    ...
    for serie_idx, serie in enumerate(series):
        ...
        if save_directory is not None:
            ...
            # Lưu chỉ các ảnh có attention vượt ngưỡng
            for idx, (img, attention) in enumerate(
                zip(overlayed_images, cur_attention)
            ):
                if np.max(attention) > attention_threshold:  # Kiểm tra ngưỡng attention
